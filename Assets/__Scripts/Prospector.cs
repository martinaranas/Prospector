...
public class Prospector : MonoBehaviour {
	...
	void Awake(){
		...
		// Check for a hgh score in PlayerPrefs
	...
	void LayoutGame(){
		...
		foreach (SlotDef tSD in layout.slotDefs){
			...
			// This replaces the previous parent: deck.Anchor, which appears
			// as _Deck in the Hierachy when the scene is playing.
	...
	void MoveToDiscard(CardProspector cd) {
		// Set the state of this card to discard
	...
	void MoveToTarget(CardProspector cd) {
		// If there is currently a target a card, move it to the discardPile
		if (target != null)
	...
	// Arranges all the cards of the drawPile to show many are left
	void UpdateDrawPile() {
		CardProspector cd;
	...
	public bool AdjacentRank (CardProspector cO, CardProspector cl) {
		...
		if (!cO.faceUp || !cl.faceUp)
			return(false);
		// if they are 1 apart, they are adjacent
		if (Mathf.Abs (cO.rank - cl.rank) == 1) {
			return(true);
		}
		// If one is A and the other King, they're adjacent
		if (cO.rank == 1 && cl.rank == 13)
			return (true);
		if (cO.rank == 13 && cl.rank == 1)
			return (true);
